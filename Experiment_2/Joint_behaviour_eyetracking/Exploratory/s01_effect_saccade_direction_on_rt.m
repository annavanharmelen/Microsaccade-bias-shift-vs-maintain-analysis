...
% =========================================================================
% SCRIPT: s01_effect_saccade_direction_on_rt.m (Exploratory)
...
for pp = pp2do
    ...
    oktrials = ismember(behdata.broke_fixation, {'False'})&ismember(behdata.premature_pressed, {'False'});

    % select trials broken after target change
    also_oktrials = ismember(behdata.exit_stage, {'orientation_change'})&ismember(behdata.premature_pressed, {'False'});
...
figure;
hold on
bar(mean(reaction_time_saccade([1:18,21:24], :)));
errorbar([1, 2, 3], mean(reaction_time_saccade([1:18,21:24], :)), (std(reaction_time_saccade([1:18,21:24], :)) ./ sqrt(size(pp2do, 2))), "LineStyle","none");
xticks([1, 2, 3])
xticklabels({'toward saccade', 'away saccade', 'no saccade'})
title('About saccades in shift window and valid trials only');

%% stats
[h,p,ci,stats] = ttest(reaction_time_saccade([1:18,21:24], 1), reaction_time_saccade([1:18,21:24], 2))
