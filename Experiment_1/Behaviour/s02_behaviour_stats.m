...
for soa = 1:size(trial_lengths, 2)
    ...
    [h,p,ci,stats] = ttest(reaction_time_per_soa_valid(:,soa), reaction_time_per_soa_invalid(:,soa));
    disp(['rt: ', 'p=', num2str(p)])
    disp(stats)
    [h,p,ci,stats] = ttest(accuracy_per_soa_valid(:,soa), accuracy_per_soa_invalid(:,soa));
